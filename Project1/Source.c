...
int princess_counter = 0, heap_size = 0, array_length = 0;
...
void map_init(char **mapa, int n, int m) {
	...
	for (int i = 0; i < m; i++) {
		for (int j = 0; j < n; j++) {
			...
			if (mapa[i][j] == 'P' && princess_counter == 3) {
				map[i][j].princess = PRINCESS_4;
				princess_counter++;
				continue;
			}
			if (mapa[i][j] == 'P' && princess_counter == 4) {
				map[i][j].princess = PRINCESS_5;
				princess_counter++;
				continue;
			}
		}
...
char **permute(char *to_permute, int length, int num) {

	int x, permutation_counter = 0;
	char **permutations = NULL;

	permutations = (char **)malloc(num * sizeof(char *));
	for (int i = 0; i < num; i++)
	...
	if (length > 1) {
		for (int i = 0; i < 3; i++) {
			char temp;
			temp = to_permute[0];
			to_permute[0] = to_permute[x];
			to_permute[x] = temp;
			for (int j = 0; j < length; j++) {
				permutations[permutation_counter][j] = to_permute[j];
			}
			permutation_counter++;
			if (num == permutation_counter) break;
			temp = to_permute[1];
			to_permute[1] = to_permute[x];
			to_permute[x] = temp;
			for (int j = 0; j < length; j++) {
				permutations[permutation_counter][j] = to_permute[j];
			}
...
int *zachran_princezne(char **mapa, int m, int n, int t, int *dlzka_cesty) {
	...
	if (princess_counter == 1) {
		char to_permute[] = "1";
		permutations = permute(to_permute, 1, 2);
		perm_num = 1;
	}
	if (princess_counter == 2) { 
		char to_permute[] = "12";
		permutations = permute(to_permute, 2, 4);
		perm_num = 4;
	}
	if (princess_counter == 3) { 
		char to_permute[] = "123";
		permutations = permute(to_permute, 3, 6);
		perm_num = 6;
	}
	if (princess_counter == 4) { 
		char to_permute[] = "1234";
		permutations = permute(to_permute, 4, 6);
		perm_num = 6;
	}
	if (princess_counter == 5) {  
		char to_permute[] = "12345";
		permutations = permute(to_permute, 5, 6);
		perm_num = 6;
	}
		...
		for (int i = 0; i < princess_counter; i++) {
			while (TRUE) {
				...
				if (map[x][y].princess == permutations[try][i]) {
					//printf("Uf, uz som presiel %d.\n", map[x][y].lenght);
					length_final += map[x][y].lenght;
					printf("Je tu teta, musim ju zachranit. %d %d\n", x, y);
					break;
				}
				dijkstra(priority_queue, x, y, length, n, m);
			}
	...
	for (int i = 0; i < lengths[index_to_return] / 2; i++)
